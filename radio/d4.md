...
Isn't there already a perfectly fine station list, written by our beloved 0x23 command?\
...
Like your beautiful carefully crafted 0x23 list command that the BM24 uses for its station list!\
That would never work with such a small screen!

But your boss says: "JA! Zat sounds like a great ideaa! Zat schouldn't be a big deal."
...
The contents are exclusively used in the FMD mode of the BM54 **and** GT splitscreen mode.

The problem with the normal 0x23 command is that it was designed for a screen layout that could fit 10 list items.\
But the splitscreen view can only fit up to 6 items on a screen!\
That's fine for the BM53 for example, because it only has to display 6 items at most, due to lack of a second tuner, and
therefore it doesn't support a full station list.

The BM54 on the other hand writes a full screen with stations and even a next button.\
It would be a very awkward UI if you would fit 10 items (including '>>') on a 6 items layout with a scrollbar. (This is
actually supported by the way!)\
To resolve this issue, the NG-Radio Station List command was created.
The GT buffers the contents and uses it to build an endless scrolling station list.
