...
That's fine for the business radios (BM23, BM53), because it only has to display 6 items at most, due to lack of a second tuner, and
therefore it doesn't support a full station list.

The professional radios (BM24, BM54[^1]) on the other hand write a full screen with stations and even a next button.\
...
[^1] Actually, the BM54 never uses the `0x21` command in FMD mode, even when the splitscreen is disabled.\
