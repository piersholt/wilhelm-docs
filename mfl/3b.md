...
GT also listens for this command directly.\
In this path it only checks `BUTTON_RT`. A zero value is handled as the opposite R/T state.

When addressed to Telephone `0xc8`, `BUTTON_TEL` is also used by the telephone UI.
